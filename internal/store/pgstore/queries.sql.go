...
const getRoomsMessages = `-- name: GetRoomsMessages :many
SELECT
    "id", "room_id", "message", "reaction_count", "answered"
FROM messages
`

func (q *Queries) GetRoomsMessages(ctx context.Context) ([]Message, error) {
	rows, err := q.db.Query(ctx, getRoomsMessages)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
